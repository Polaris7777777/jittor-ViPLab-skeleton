...
class FurthestPointSampler(nn.Module):
    ...
    def execute_with_indices(self, x):
        '''
        Parameters
        ----------
        x: jt.Var, (B, N, 3)
        ...
        Returns
        -------
        idxs: jt.Var, (B, n_samples) - 如果n_samples>N,会重复使用某些点的索引
        '''
        batch_size, n_points, n_coords = x.shape
        
        if self.n_samples <= n_points:
            ...
            block_size = optimal_block(batch_size)
            cuda_src = self.cuda_src.replace('#block_size', str(block_size))
            base_idxs = jt.code([batch_size, n_points], 'int32', [x,], cuda_src=cuda_src)
            
            # 2. 随机重复一些点的索引来达到需要的数量
            remaining = self.n_samples - n_points
            # 从已有索引中随机选择remaining个
            repeat_idxs = jt.random([batch_size, remaining]).int() % n_points
            idxs = jt.contrib.concat([base_idxs, repeat_idxs], dim=1)
